...
// Migrate applies the embedded rex job-store migrations to a PostgreSQL pool.
func Migrate(ctx context.Context, pool *pgxpool.Pool) error {
	if pool == nil {
		return errors.New("postgres migration pool is required")
	}
	...
	store, err := database.NewStore(goose.DialectPostgres, gooseTableName)
	if err != nil {
		return fmt.Errorf("create migration store: %w", err)
	}
	provider, err := goose.NewProvider(goose.DialectCustom, sqlDB, fsys, goose.WithStore(&migrationStore{Store: store}))
	if err != nil {
		return fmt.Errorf("create migration provider: %w", err)
	}
	if err := validateGooseVersions(ctx, sqlDB, provider); err != nil {
		return err
	}
